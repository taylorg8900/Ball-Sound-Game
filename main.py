...
from ball import make_ball, make_spawner, make_speed_text
...
SPAWNER_TICKS = 500 # not sure how this is measured tbh so imma set it to clock delay so its maybe once a second
...
def main():
    ...
    bamboo_noise = pygame.mixer.Sound("sounds/bamboo.wav")
    bamboo_noise.set_volume(.1)
    ...
    while running:
        ...
        for spawner in spawner_list:
            ...
            if mouse_touching and key_4:
                spawner.interval -= 50
                if spawner.interval < 50:
                    spawner.interval = 50
                else:
                    # text
                    pass
                    spawner_text = make_speed_text(spawner.centerx,spawner.centery,"speed up")
                    text_list.append(spawner_text)

            # slower
            elif mouse_touching and key_5:
                spawner.interval += 50
                if spawner.interval > 1000:
                    spawner.interval = 1000
                else:
                    # text
                    pass
                    spawner_text = make_speed_text(spawner.centerx,spawner.centery,"speed down")
                    text_list.append(spawner_text)
        ...
        for ball in ball_list:
            for line in line_list:
                if ball.immune == False:
                    ...
                    if line.mask.overlap(ball.mask, (offset_x,offset_y)) and line.slant == "left facing":
                        ...
                        hit_line = True
                        '''
                screen.blit(line.surface, [line.left_node_x, line.top_left_y])
            ...
            print(f"amount of balls: {len(ball_list)}")
            '''
        ...
        for text in text_list:
            text.update_text()
            if text.text_rect.y < 0:
                text_list.remove(text)
            screen.blit(text.text, text.text_rect)



        font = pygame.font.SysFont("timesnewroman",20)
        text = font.render("esc to quit\n1 = ball\n2 = node\n3 = spawner\n4 = speedup\n5 = slowdown\n6 = toggle noise",True,"white")
